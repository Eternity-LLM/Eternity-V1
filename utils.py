...
def ssd(dt:torch.Tensor, X:torch.Tensor, A:torch.Tensor, B:torch.Tensor, C:torch.Tensor, block_len:int = 64, initial_states:Optional[torch.Tensor] = None):
    return MambaChunkScanCombined.apply(X, dt, A, B, C, block_len, initial_states=initial_states, return_final_states=True)
